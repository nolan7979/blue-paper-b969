...
const SnookerResultsPage =({ data, isDesktop, dataAPI, featureData }: MetaProps) => {
  ...
  return (
    <>
      ...
      <LiveScoreComponent page={PAGE.results} sport={SPORT.SNOOKER} 
...
// SnookerResultsPage.Layout = MainLayout;

SnookerResultsPage.getInitialProps = async (context: GetServerSidePropsContext) => {
  const { res } = context;
  let isDesktop = true;
  ...
  try {
    ...
    const configv2 = {
      ...
      url: `${process.env.NEXT_PUBLIC_API_BASE_URL}/snooker/featured-events`,
      headers: {},
    };
    ...
    const config = {
      method: 'get',
      maxBodyLength: Infinity,
      url: `${process.env.NEXT_PUBLIC_API_BASE_URL}/sport/snooker/scheduled-events/${today}/offset/${getTimezoneOffset}`,
      headers: {},
    };
...
export default SnookerResultsPage;
