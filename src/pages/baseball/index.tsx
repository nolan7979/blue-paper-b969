...
const BaseballPage = ({ data }: MetaProps) => {
  return (
    <>
      <Seo {...getMetaContent(data)} />
      <BaseballSubPage sport={SPORT.BASEBALL} page={PAGE.liveScore} />
      {!data?.notFound && <Tabs data={data} />}
    </>
  );
};

BaseballPage.Layout = BaseballSubPage.Layout;

export default BaseballPage;

BaseballPage.getInitialProps = async (context: GetServerSidePropsContext) => {
  try {
    const locale = getLocaleSEOContext(context.locale);

    const data = await getContentStaticPage(
      CONTENT_SLUG[locale as 'vi' | 'en'].HOME_BASEBALL,
      locale
    );
