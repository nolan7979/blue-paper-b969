...
import LeagueDetailedSubPage from '@/modules/baseball/competition/page';
...
LeagueDetailedPage.getInitialProps = async (
  ...
  const url = `${getAPIDetectedIP()}/baseball/unique-tournament/${competitionId}`;
  const config = {
    method: 'get',
  ...
  const data = await getContentStaticPage(
    CONTENT_SLUG[locale as 'vi' | 'en'].STANDINGS_BASEBALL,
    locale
  );
  ...
  const urlSeason = `${getAPIDetectedIP()}/baseball/unique-tournament/${competitionId}/seasons`;
  const configSeason = {
    method: 'get',
    maxBodyLength: Infinity,
    url: urlSeason,
