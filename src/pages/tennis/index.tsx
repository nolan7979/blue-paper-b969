...
const TennisPage = ({ data }: MetaProps) => {
...
TennisPage.Layout = MainLayout;

TennisPage.getInitialProps = async (context: GetServerSidePropsContext) => {
  try {
    const locale = getLocaleSEOContext(context.locale);
    const data = await getContentStaticPage(
      CONTENT_SLUG[locale as 'vi' | 'en'].HOME_TENNIS,
      locale
    );
