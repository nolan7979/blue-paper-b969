...
import LeagueDetailedSubPage from '@/modules/badminton/competition/page';
...
LeagueDetailedPage.getInitialProps = async (
  ...
  const url = `${getAPIDetectedIP()}/badminton/unique-tournament/${competitionId}`;
  const config = {
    method: 'get',
  ...
  const data = await getContentStaticPage(
    CONTENT_SLUG[locale as 'vi' | 'en'].STANDINGS_BADMINTON,
    locale
  );
  ...
  const urlSeason = `${getAPIDetectedIP()}/badminton/unique-tournament/${competitionId}/seasons`;
  const configSeason = {
    method: 'get',
    maxBodyLength: Infinity,
    url: urlSeason,
  ...
  // console.log(competitionData, seasonData)
