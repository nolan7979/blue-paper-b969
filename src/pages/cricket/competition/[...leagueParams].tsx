...
const LeagueDetailedSubPage = dynamic(
  () => import('@/modules/cricket/competition/page'),
  { ssr: false }
);
...
LeagueDetailedPage.getInitialProps = async (
  ...
  const url = `${getAPIDetectedIP()}/cricket/unique-tournament/${competitionId}`;
  const config = {
    method: 'get',
  ...
  const data = await getContentStaticPage(
    CONTENT_SLUG[locale as 'vi' | 'en'].STANDINGS_CRICKET,
    locale
  );
  ...
  const urlSeason = `${getAPIDetectedIP()}/cricket/unique-tournament/${competitionId}/seasons`;
  const configSeason = {
    method: 'get',
    maxBodyLength: Infinity,
    url: urlSeason,
