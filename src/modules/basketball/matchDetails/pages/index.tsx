...
import { QuickViewColumn as BkbQuickViewColumn } from '@/components/modules/basketball/quickviewColumn';

import { useFilterStore, useHomeStore, useMatchStore } from '@/stores';
import { useIsConnectSocketStore } from '@/stores/is-connect-socket';

import { MatchListIsolated } from '@/components/modules/basketball/match';
...
import { isMatchLive, parseMatchDataBasketBall } from '@/utils';
...
const MatchDetails: React.FC<MatchDetailsProps> = ({ matchData, matchId }) => {
  ...
  const { data: liveMatches } = useLiveMatchData(SPORT.BASKETBALL);
  ...
  return (
    <>
      <div className='hidden py-4 lg:block'>
        {matchData && (
          <BreadCrumbMain matchData={matchData} sport={SPORT.BASKETBALL} />
        )}
      </div>
      <div className='grid w-full grid-cols-12 gap-4 '>
        <div className='top-0 col-span-4 hidden overflow-y-auto no-scrollbar lg:sticky lg:col-span-3 lg:block lg:h-[91vh]'>
          ...
          <TwFilterCol className='flex-shrink-1 rounded-md '>
            <FilterColumn sport={SPORT.BASKETBALL} isDetail={true} />
          </TwFilterCol>
        </div>
