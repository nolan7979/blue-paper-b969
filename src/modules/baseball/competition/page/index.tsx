...
import { useFeaturedMatch } from '@/hooks/useBaseball';
...
const LeagueDetailedSubPage: React.FC<LeagueDetailedSubPageProps> = ({
  ...
  const { data: topLeagues } = useTopLeagues('baseball');
    ...
    <>
      <div className='layout hidden lg:block'>
        <BreadCrumb className='no-scrollbar overflow-x-scroll py-5'>
          ...
          <BreadCrumbSep />
          <BreadCumbLink
            href={`/baseball/competition/${getSlug(uniqueTournament?.short_name || uniqueTournament?.name)}/${uniqueTournament?.id}`}
          ...
          <TwMainCol className='!col-span-2'>
            <div className='h-full overflow-hidden'>
              <div className='flex w-full flex-col gap-6'>
                ...
                />
                <MatchesBB seasonId={selectedSeason?.id} />
                <StandingStableBB
                  uniqueTournament={uniqueTournament}
                  selectedSeason={selectedSeason}
                />
                <CupTreeBB selectedSeason={selectedSeason} />
              </div>
            </div>
          </TwMainCol>
