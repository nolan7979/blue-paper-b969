...
const MatchDetails: React.FC<MatchDetailsProps> = ({ matchData, matchId }) => {
  ...
  const { data: liveMatches } = useLiveMatchData(SPORT.BASEBALL);

  useEffect(() => {
    let liveMatchesConvert = {};
    liveMatchesConvert =
      parseMatchDataArrayBaseball(liveMatches as string)?.reduce(
        (acc, match) => {
  ...
  useEffect(() => {
    if (
      !isConnectSocket &&
      isMatchInprogressBB(matchData ? matchData.status.code : -1)
    ) {
