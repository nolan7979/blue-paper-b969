...
export const TopPlayerPerGameSection = ({
  ...
  if (isLoading || !data) {
    ...
    return (
      <div className='flex h-fit w-full  flex-col gap-3 rounded-lg bg-dark-container px-2 py-8'>
  ...
  return (
    <div className='rounded-lg bg-dark-container'>
  ...
  return (
    <>
      <div className=' flex gap-0.5'>
        <div className='no-scrollbar w-5/12 flex-1 overflow-scroll py-2.5 xl:p-2.5'>
          <div
...
export const RoundFilterSection = ({
  uniqueTournamentId,
  selectedSeasonId,
}: {
  uniqueTournamentId: string;
  ...
  const i18n = useTrans();
  const { selectedRound, setSelectedRound } = useLeagueMatchFilterStore();

  useEffect(() => {
    if (!isLoading && !isError && data) {
      const { currentRound } = data;

      if (currentRound && currentRound.stage_id && currentRound.name) {
        setSelectedRound(currentRound);
      }
    }
  }, [isLoading, isError, data, setSelectedRound]);
  ...
  const handleNextClick = () => {
    const currentRoundIndex = rounds.findIndex(
      (round: IRoundItemV2) =>
        ...
        round.stage_id === selectedRound.stage_id
    );
  ...
  return (
    <>
      <div className='flex w-full items-center justify-between gap-2 p-2.5'>
        <div className='w-1/4'>
          <TwFilterButton onClick={handlePreviousClick}>
        ...
        <div className='w-1/4'>
          <TwFilterButton onClick={handleNextClick}>
...
export const GroupMatches = ({
  ...
  return (
    <div className='rounded-md bg-light-match px-0.5 dark:bg-dark-match md:p-2 hidden lg:block'>
...
export const StandingTypeFilterGroup = ({
  ...
  return (
    <div className='flex justify-between gap-x-2 lg:justify-start'>
      <div className='flex gap-3 bg-dark-head-tab rounded-full'>
        {filterOptions.map((option) => (
          <FilterButton
            key={option.value}
            label={option.label}
            isActive={bxhData === option.value}
            onClick={() => setBxhData(option.value)}
          />
        ))}
      </div>
