...
const Matches: React.FC<any> = ({ seasonId }) => {
  ...
  const matchData = [...matchDataLast, ...matchDataNext].sort((a:any, b:any) => {
  ...
  const onClick = (match: SportEventDtoWithStat) => {
    router.push(`/${SPORT.SNOOKER}/match/${match.slug}/${match.id}`)
  };
