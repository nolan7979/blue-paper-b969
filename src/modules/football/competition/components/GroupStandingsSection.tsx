...
const GroupStandingsSection: React.FC<{
  ...
  forTeam = false,
  isLastGroup = false,
  isStandingTabSport=false
}) => {
  const i18n = useTrans();
  const { bxhFormat, bxhData, toggleRecentMatch } = useFilterStore();
  ...
  return (
    <>
      <div className='overflow-x-auto rounded-lg border border-light-theme dark:border-head-tab scrollbar bg-white dark:bg-dark-head-tab'>
        ...
        <ul className={`divide-list text-xs ${toggleRecentMatch ? 'w-full' : 'w-fit'}`}>
