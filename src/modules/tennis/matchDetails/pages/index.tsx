...
import {
  QuickViewColumn as BkbQuickViewColumn,
  MatchListIsolated,
...
import { isMatchLiveTennis, parseMatchDataArrayTennis } from '@/utils';
...
const MatchDetails: React.FC<MatchDetailsProps> = ({ matchData, matchId }) => {
  ...
  const { mbDetailMatchTab, setMbDetailMatchTab, setDateFilter } =
    useFilterStore();
  ...
  const {
    data: liveMatches,
    refetch: refetchMatchLive,
    isRefetching,
  } = useLiveMatchData(SPORT.TENNIS, parseMatchDataArrayTennis);

  useEffect(() => {
    if (liveMatches) setMatchesLive(liveMatches);
  }, [liveMatches]);

  useEffect(() => {
    if (!isConnectSocket && Object.keys(matchesLive).length > 0) {
  ...
  }, [isConnectSocket, matchesLive, refetchMatchLive]);

  useEffect(() => {
    if (
      !isConnectSocket &&
      isMatchLiveTennis(matchData ? matchData.status.code : -1)
    ) {
      const intervalId = setInterval(() => {
        refetch();
      }, 5000);
  ...
  return (
    <>
      <div className='hidden py-4 lg:block'>
        {matchData && (
          <BreadCrumbMain matchData={matchData} sport={SPORT.TENNIS} />
        )}
      </div>
        ...
        <div className='top-0 col-span-4 hidden overflow-y-auto no-scrollbar lg:sticky lg:col-span-3 lg:block lg:h-[91vh]'>
          <div className='hidden xl:block'>
            <MatchListIsolatedSocket
              ...
              isRefeshing={isRefetching}
            />
            <TwFilterCol className='flex-shrink-1 rounded-md '>
              <FilterColumn sport={SPORT.TENNIS} isDetail={true} />
            </TwFilterCol>
          </div>
        </div>
...
export const MatchListIsolatedSocket: React.FC<{
  ...
  setMatchesLive: any;
  isRefeshing?: boolean;
}> = ({ sport, matchesLive, setMatchesLive, isRefeshing }) => {
  ...
  return (
    <MatchListIsolated
      ...
      isLiveMatchRefetching={isRefeshing}
    />
  );
};
