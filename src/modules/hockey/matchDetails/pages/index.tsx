...
import { MatchListIsolated } from '@/components/modules/hockey/match';
...
const MatchDetails: React.FC<MatchDetailsProps> = ({
  ...
  const { data: liveMatches } = useLiveMatchData(SPORT.ICE_HOCKEY);
  ...
  //   if (
  //     !isConnectSocket &&
  //     isInProgessMatchHockey(matchData ? matchData.status.code : -1)
  //   ) {
  ...
  return (
    <>
      <div className='hidden py-4 lg:block'>
        {matchData && (
          <BreadCrumbMain matchData={matchData} sport={SPORT.ICE_HOCKEY} />
        )}
      </div>
          ...
          <>
            <div className='top-0 col-span-4 hidden overflow-y-auto no-scrollbar lg:sticky lg:col-span-3 lg:block lg:h-[91vh]'>
              ...
              <TwFilterCol className='flex-shrink-1 rounded-md '>
                <FilterColumn sport={SPORT.ICE_HOCKEY} isDetail={true} />
              </TwFilterCol>
            </div>
