...
const QuickViewSummaryLeague = () => {
  const i18n = useTrans();
  const { selectedFavorite } = useMatchStore();

  const { data, refetch } = useFetchLeagueData(
    selectedFavorite?.id || '', selectedFavorite?.sport || SPORT.FOOTBALL
  );
  
  const leagueDetail = data && data?.uniqueTournament ? data?.uniqueTournament : data;
  ...
  return (
    <div className='dark:border-linear-box relative dark-away-score lg:custom-bg-white dark:lg:bg-primary-gradient rounded-md px-4 py-6'>
      <div className='absolute right-3 top-3 z-10'>
        <CustomLink href={`/${selectedFavorite?.sport}/competition/${getSlug(leagueDetail?.name)}/${selectedFavorite?.id}`} target='_parent'>
          <TwButtonIcon
            icon={<ZoomOutSVG />}
      ...
      <div className='flex items-center gap-3'>
        <Avatar id={leagueDetail?.id} type='competition' width={90} height={90} sport={selectedFavorite?.sport} />
        <div>
...
export default QuickViewSummaryLeague;
