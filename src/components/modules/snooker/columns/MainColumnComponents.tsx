  ...
  return (
    <>
      <div className='sticky top-[7rem] z-[5] flex justify-between lg:relative lg:top-0'>
        <div className='flex w-full items-center border border-light-match bg-white p-2.5 dark:border-none dark:bg-dark-score md:pb-0 lg:hidden'>
          <WeekView />
        </div>
...
export const MatchListHeader = React.memo(() => {
  ...
  return (
    <TwMatchListContainer className='h-auto !px-0 text-csm dark:bg-dark-wrap-match dark:lg:bg-transparent'>
      <TwMatchHeader className='py-1.5'>
        ...
        <TwTeamScoreCol className=''>
          <TwTitleHeader className=''>{i18n.titles.player}</TwTitleHeader>
        ...
        <div className='mr-4 flex gap-1 xl:gap-2.5'>
...
const LeagueRow = React.memo(
  ({ match, isLink }: { match: SportEventDtoWithStat; isLink: boolean }) => {
    ...
    const isTournamentFollowed = (
      tournament: TournamentDto | any,
      followedTournaments: any
    ) => {
      const tournamentSport = followedTournaments['snooker'] || [];
    ...
    const changeFollow = useCallback(() => {
      if (!isFollowedTour) {
        addTournament('snooker', newTournament);
      } else {
        removeTournament('snooker', newTournament);
      }
          ...
          {category?.name && category?.id && (
            <div className='text-msm dark:text-dark-text'>
              {isCountryName(category?.name) && (
                <CustomLink
                  href={`/football/country/${getSlug(category?.name)}/${
                    category?.id
                  }`}
                  className='hover:text-logo-blue'
                  disabled
  ...
  return (
    <>
      <TwMatchListContainer className=''>
        ...
        {loadMoreMatches && (
          <MoreMatchesLoader getMatches={setMoreMatches} sport={sport} />
        )}
      </TwMatchListContainer>
    </>
  );
