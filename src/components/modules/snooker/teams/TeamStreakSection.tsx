...
const TeamStreakSection = ({
  ...
  return (
    <TwMbQuickViewWrapper className='space-y-2 dark:bg-dark-card rounded-lg'>
      <TwQuickViewTitleV2 className='pl-2 uppercase lg:pl-0 pt-4'>
      ...
      <TwQuickViewSection className='mx-2 rounded-md lg:mx-0 lg:p-4'>
        <ul className='divide-list text-sm'>
          {general.map((item: ITeamStreaksItem, index: number) => {
            ...
            return (
              <TeamStreakRow key={index} item={item} team1={competitor} i18n={i18n}/>
            );
          })}
        </ul>
      </TwQuickViewSection>
