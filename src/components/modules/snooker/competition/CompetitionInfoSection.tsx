import { TwCard, TwTitle } from '@/components/modules/football/tw-components';
import HostSection from '@/components/modules/football/competition/HostSection';
import InfoList from '@/components/modules/football/competition/InfoList';
import MostTitleHolderSection from '@/components/modules/football/competition/MostTitleHolderSection';
import TitleHolderSection from '@/components/modules/football/competition/TitleHolderSection';

const CompetitionInfoSection = ({
  uniqueTournament,
  i18n,
}: {
  uniqueTournament: any;
  i18n: any;
}) => {
  const {
    mostTitles = 0,
    mostTitlesTeams = [],
    titleHolderTitles = 0,
    titleHolder = {},
    upperDivisions,
    upperNewComers,
    lowerDivisions,
    lowerNewComers,
    host,
  } = uniqueTournament || {};

  const baseClassName = 'flex flex-col justify-start items-start [&>*:not(:last-child)]:border-gray-700 [&>*:not(:last-child)]:border-b [&>*:not(:last-child)]:border-dotted [&>*]:w-full [&>*:not(:last-child)]:pb-2.5 gap-y-3'
  // if (
  //   upperDivisions.length == 0 &&
  //   upperNewComers.length == 0 &&
  //   lowerNewComers.length == 0
  // )
  //   return <></>;
  return (
    <div className='space-y-4 p-2.5 lg:px-0'>
      <div className='space-y-8 lg:space-y-6'>
        <div className='flex w-full flex-wrap items-start justify-center gap-x-18 rounded-md bg-transparent dark:bg-transparent lg:bg-light lg:dark:bg-dark-card'>
          <div className='w-full py-3.5 text-center'>
            <TwTitle>{i18n.competition.tournamentInfo}</TwTitle>
          </div>
          <div className='block lg:hidden pb-1 text-sm font-bold text-black dark:text-white w-full text-left mb-3'>{i18n.competition.titleHolder}</div>
          <div className='w-full flex justify-center gap-x-18 p-4 bg-light dark:bg-dark-card lg:bg-transparent lg:dark:bg-transparent rounded-md'>
            <TitleHolderSection
              titleHolder={titleHolder}
              titleHolderTitles={titleHolderTitles}
              i18n={i18n}
            />
            {/* <MostTitleHolderSection
              mostTitles={mostTitles}
              mostTitlesTeams={mostTitlesTeams}
              i18n={i18n}
            /> */}
          </div>
        </div>
        <div className='flex flex-col lg:flex-row justify-center items-start w-full gap-x-6 gap-y-8 lg:gap-y-6'>
          <InfoList
            newComers={lowerDivisions}
            type='competition'
            title={i18n.competition.lowerDivision}
          />

          <InfoList
            newComers={upperDivisions}
            type='competition'
            title={i18n.competition.upperDivision}
          />
        </div>

        <InfoList
          newComers={upperNewComers}
          type='team'
          className={baseClassName}
          title={i18n.competition.upperNewComers}
        />

        <InfoList
          newComers={lowerNewComers}
          type='team'
          className={baseClassName}
          title={i18n.competition.lowerNewComers}
        />

        {/* TODO: data  */}
        {/* <FactSection></FactSection> */}

        {/* <HostSection i18n={i18n} host={host}></HostSection> */}
      </div>
    </div>
  );
};

export default CompetitionInfoSection;
