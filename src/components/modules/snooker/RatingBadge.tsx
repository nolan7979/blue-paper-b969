  ...
  return (
    <div
      className={clsx(
        className,
        color +
          ' flex place-content-center items-center rounded-cmd px-[1px] font-oswald text-[10px] font-black leading-3 tracking-tighter'
      )}
      css={[isSmall && tw`h-5 w-fit`, !isSmall && tw`h-6 w-fit`]}
    >
      {/* <span className='px-1 text-white' test-id='team-rate'>{roundNumber(point, 1)}</span> */}
      <span className='px-1 text-white' test-id='team-rate'>{point}</span>
    </div>
  );
};
