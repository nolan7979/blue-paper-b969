...
const useMatchTimestamp = (
  ...
  const [dateTimeZone, setDateTimeZone] = useState(() => {
    const [dateStr] = formatMatchTimestampSnooker(
      startTimestamp,
      status || {},
      false, 
  ...
  const [timeTimeZone, setTimeTimeZone] = useState(() => {
    const [, timeStr] = formatMatchTimestampSnooker(
      startTimestamp,
      status || {},
      false,
  ...
  const updateTimestamps = useCallback(() => {
    const [newDateStr, newTimeStr] = formatMatchTimestampSnooker(
      startTimestamp,
      status,
      false,
      currentPeriodStartTimestamp
  ...
  }, [startTimestamp, status, status.code, currentPeriodStartTimestamp, id, addMore, timeTimeZone]);
  ...
  }, [currentPeriodStartTimestamp, status.type,status.code, updateTimestamps]);
        ...
        <TwTimeCol dir='ltr' className='flex h-full flex-col gap-1'>
          ...
          <span
            className={clsx(
              ...
              status.type === 'inprogress' || isMatchHaveStatSnooker(status.code)
                ? 'font-medium text-minute dark:text-dark-green'
                : 'text-black dark:text-white'
            )}
            test-id='time-match'
          >
            {status.code === 30 ? 'FT' :renderTimeZone(timeTimeZone)}
          </span>
        </TwTimeCol>
      </FirstCol>
    );
  }, [dateTimeZone, isInprogress, status.type, status.code, timeTimeZone]);
