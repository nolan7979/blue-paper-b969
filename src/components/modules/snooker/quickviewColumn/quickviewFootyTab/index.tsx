import BothTeamsScore, {
  BothTeamsScoreData,
} from '@/components/modules/football/quickviewColumn/quickviewFootyTab/BothTeamsScore';
import CardFullTime, {
  CardFullTimeData,
...
import ShotsTaken, { formatShotsTakenData } from '@/components/modules/football/quickviewColumn/quickviewFootyTab/ShotsTaken';
...
const QuickVewFooty: FC<QuickVewFootyProps> = ({ matchData }) => {
  ...
  const formattedData = useMemo(() => {
    if (!data) return {};
    const isHomeCup = data?.home_team_info?.stats?.season_format === 'cup';
    const isAwayCup = data?.away_team_info?.stats?.season_format === 'cup';
    return {
      summaryData: {
