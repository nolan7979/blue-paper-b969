...
import { LoadMoreMatchesBtn } from '@/components/modules/cricket/loaderData/LoadMoreMatchesBtn';
import { MatchRow } from '@/components/modules/cricket/match/MatchRow';
import { MatchRowByTime } from '@/components/modules/cricket/match/MatchRowByTime';
import {
  TwBellCol,
  TwMatchHeader,
  TwScoreColHeader,
    ...
    <TwMatchListContainer className='h-auto !px-0 text-csm dark:bg-dark-wrap-match dark:lg:bg-transparent'>
      <TwMatchHeader className=''>
        <div className='flex w-14 flex-col-reverse items-center justify-evenly md:flex-row lg:w-12'>
          ...
          <TwTitleHeader className='capitalize'>
            {i18n.time.hours}
          </TwTitleHeader>
        </div>
...
const LeagueRow = React.memo(
  ({ match, isLink }: { match: SportEventDtoWithStat; isLink: boolean }) => {
    ...
    const tournament = uniqueTournament?.tournament;
    ...
    const buildCompetitionLink = useCallback(
      () =>
        `/cricket/competition/${uniqueTournament?.slug}/${
          ...
          {tournament &&
            ...
            tournament?.category?.id && (
              <div className='text-msm dark:text-dark-text'>
                {/* {isCountryName(tournament?.category?.name) && (
                <Link
                  href={`/cricket/country/${getSlug(tournament?.category?.name)}/${tournament?.category?.id
                    }`}
                  className='hover:text-logo-blue'
                >
                  {tournament?.category?.name}
