...
export const LiveOddsCompareTableReprTX = ({
  matchId,
  half = 0,
}: {
  matchId: string;
  ...
  const { oddsTXCompareData, oddsTXCompareDataHT } = useOddsDetailsStore();
  ...
  const shownData = half === 0 ? oddsTXCompareData : oddsTXCompareDataHT;
...
const OddsDetailRowTX = ({
  ...
  const { oddsTXCompareData, oddsTXCompareDataHT } = useOddsDetailsStore();
  const bookData =
    (half === 0 ? oddsTXCompareData[bookId] : oddsTXCompareDataHT[bookId]) ||
    {};
  const { tx } = bookData || {};
