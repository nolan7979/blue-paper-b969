...
interface TeamAwayLineUpHorizontalProps {
  lineups: any;
  mapPlayerEvents: any;
  matchData: any;
}

const TeamAwayLineUpHorizontal: React.FC<TeamAwayLineUpHorizontalProps> = ({
  lineups,
  mapPlayerEvents,
  matchData,
}) => {
  ...
  return (
    <div
      className={clsx(
        'aspect-h-1 aspect-w-1 rounded-r-md bg-contain bg-no-repeat',
        { 'lg:relative lg:right-3': linePlayers.length > 4 }
      )}
    >
      <div
        className={clsx(
          'flex w-full justify-evenly px-2',
          linePlayers.length > 4 ? 'gap-0' : 'gap-2'
        )}
      >
...
export default TeamAwayLineUpHorizontal;
