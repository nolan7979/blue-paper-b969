...
export const QvPlayerReverse = ({
  ...
  return (
    <div className=' flex items-center justify-end gap-2'>
      <div test-id='player-shirtNo-name2' className='w-26 text-end'>
        {rating > 0 && (
...
export const FormBadgeWithHover = ({
  ...
  const matchUrl = useMemo(() => {
    if (locale === 'en')
      return `/football/match/${getSlug(homeTeam.name)}-${getSlug(
        awayTeam.name
      )}/${id}`;

    return `/${locale}/football/match/${getSlug(homeTeam.name)}-${getSlug(
      awayTeam.name
    )}/${id}`;
  ...
  return (
    ...
    >
      {showIcon &&
        (isLink ? (
          <Link
            href={`/football/competitor/${team?.slug || team?.name}/${
              team?.id
            }`}
          >
            <Avatar
      ...
      {showName && (
        <>
          <div
            test-id='team-name'
            className={`flex w-full items-center font-primary text-csm font-normal dark:text-white ${
