...
import RoundOfMatchSection, {
  TwQWRoundOfMatchContainer,
} from '@/components/modules/cricket/quickviewColumn/quickviewDetailTab/RoundOfMatchSection';
  ...
  return (
    <div className='space-y-8'>
      <TwQVDetailTabContainer className='space-y-8 pb-8'>
        <div className='space-y-6 px-2.5 lg:space-y-8 lg:px-0'>
          <RoundOfMatchSection matchData={matchData} />
          {(!isDetail || isMobile) && <PlayByPlaySection matchData={matchData} />}
...
export const VenueSection = ({
  ...
  const {
    venue,
    uniqueTournament = {} as TournamentDto,
    startTimestamp,
  } = matchData;
  const { name = '', capacity = '', city = '' } = venue!;
  ...
  const tournamentInfo = useMemo(() => {
    ...
    return (
      <div className=''>
        <p className='text-ccsm text-black dark:text-white'>{uniqueTournament?.name}</p>
        <p className='text-ccsm'>{`${formattedDate} • ${formattedTime}`}</p>
      </div>
    );
  }, [uniqueTournament, startTimestamp]);
  ...
  return (
    ...
    <TwMbQuickViewWrapper>
      ...
      <TwQWRoundOfMatchContainer className='dark:border-linear-box p-4 dark:bg-primary-gradient'>
        <RenderIf isTrue={capacity || city || name}>
          <TwQuickViewSection className='mb-2 space-y-2 border !border-b-2'>
