...
export function SelectSeason({
  ...
  useEffect(() => {
    if (valueGetter) {
      const debouncedValueGetter = debounce(() => valueGetter(selected), 100);
      debouncedValueGetter();
    }
    ...
    >
      <Listbox value={selected} onChange={setSelected}>
        <div className='relative rounded-lg'>
          <Listbox.Button className='relative flex w-fit cursor-pointer items-center rounded-lg bg-primary-gradient bg-transparent py-1 pr-3 text-left sm:text-sm'>
          ...
          <Transition
            ...
            leaveTo='opacity-0'
          >
            <Listbox.Options className='absolute z-[9] mt-1 max-h-60 w-full overflow-auto  rounded-md bg-white py-1 text-base shadow-md  focus:outline-none dark:bg-dark-head-tab sm:text-sm'>
