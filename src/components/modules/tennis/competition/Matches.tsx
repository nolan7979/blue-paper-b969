...
import { MatchRowIsolated } from '@/components/modules/tennis';
import {
  MatchOdd,
  SeasonDto,
  SportEventDto,
...
import { useListMatchByStageTeamData } from '@/hooks/useTennis';
...
export const Matches: React.FC<MatchesProps> = ({
  ...
  const { data: listMatchData = [], isLoading: isListMatchLoading } =
    useListMatchByStageTeamData(selectedSeason?.id, stage?.id || 0);

  const onClick = (match: SportEventDtoWithStat) => {
    router.push(`/tennis/match/${match.slug}/${match.id}`);
  };
