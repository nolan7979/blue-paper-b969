...
const BracketPages = ({
  ...
  const handleClick = (match: any) => {
    const matchId = match.events[0]
    const teamSlug = `${getSlug(match.participants[0].team.name)}-vs-${getSlug(match.participants[1].team.name)}`
    if(matchId && teamSlug) {
      router.push(`/tennis/match/${teamSlug}/${matchId}`)
    }
  }
  return (
...
const Rounds = ({
  ...
  const renderTeamAvatar = (teamId: number = 0, item: any) => {
    return teamId ? (
      <>
        <HandleGroupAvatar team={item} sport={'tennis'} size={25}></HandleGroupAvatar>
        {/* <Avatar
          id={teamId.toString()}
          width={25}
          height={25}
