...
import { QuickViewFilter } from '@/modules/table-tennis/liveScore/components/QuickViewFilterMobile';
    ...
    >
      <div className='h-full no-scrollbar lg:overflow-y-scroll'>
        <div className='space-y-4 '>
          ...
          {activeTab === 'standings' && match && !!selectedSeason && (
            <>
              <StandingStableTennis selectedSeason={selectedSeason} uniqueTournament={uniqueTournament} />
            </>
          )}
          {activeTab === 'matches' && matchData && !!selectedSeason && 
            <MatchCard
              matchData={
                matchData as unknown as SportEventDtoWithStat[]
              }
            />
          }
          {activeTab === 'matchtop' && !!selectedSeason && (
              <CupTreeTTennis
                isMobile={isMobile}
