...
import { MatchTimeStamp } from '@/components/modules/tennis';
import { SPORT } from '@/constant/common';
import { SportEventDtoWithStat } from '@/constant/interface';
import MatchScoreColumn from '@/modules/football/liveScore/components/MatchScoreColumn';
import { isMatchHaveStatTableTennis, isMatchNotStartedTableTennis } from '@/utils/tableTennisUtils';
      ...
      <li
        ...
        onClick={() => {
          if (width < 1024 || isDetail) {
            ...
            router.push(`/${SPORT.TABLE_TENNIS}/match/${h2hEvent?.slug}/${h2hEvent?.id}`); // TODO slug
          } else {
      ...
      >
        ...
        <TwTeamScoreCol className=' flex w-1/4 items-center '>
          <TwTeamNameCol className=''>
            <div className=' flex items-center gap-2' test-id='club1-info'>
              <div className=' min-w-5' test-id='club1-logo'>
                <HandleGroupAvatar team={homeTeam} sport={SPORT.TABLE_TENNIS} size={20}></HandleGroupAvatar>
              </div>
              <TwTeamName>{homeTeam?.name}</TwTeamName>
            </div>
            <div className=' flex gap-2' test-id='club2-info'>
              <div className=' min-w-5' test-id='club2-logo'>
                <HandleGroupAvatar team={awayTeam} sport={SPORT.TABLE_TENNIS} size={20}></HandleGroupAvatar>
              </div>
          ...
          <div className=' flex  justify-evenly space-x-2'>
            {!isMatchNotStartedTableTennis(status?.code) && (
              <>
            ...
            {h2HFilter !== 'h2h' && isMatchHaveStatTableTennis(status?.code) && (
              <div className='flex place-content-center items-center justify-center px-2' test-id='status'>
                <FormBadge
                  isWin={isWin}
                  isLoss={isLoss}
                  isDraw={winnerCode === 3}
