    ...
    <TwBorderLinearBox className='border dark:border-0 border-line-default dark:border-linear-box bg-white dark:bg-primary-gradient'>
      <TwQuickViewSection className='space-y-3 !border-none p-4 !shadow-transparent'>
        <div className='flex items-center justify-between'>
          <p className='text-csm font-semibold text-white'>{team?.name || team?.shortName}</p>
              ...
              return (
                <span
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    window.open(
                      `/tennis/match/${getSlug(item.homeTeam.name)}-${getSlug(
                        item.awayTeam.name
                      )}/${item.id}`,
                      '_blank'
                    );
                ...
                >
                  <TwBorderLinearBox className='border-linear-form !rounded-full'>
                    <div className='flex items-center justify-center gap-x-3 rounded-full  px-2.5 py-1'>
                      <HandleGroupAvatar team={item.homeTeam} sport={SPORT.TENNIS} size={17}></HandleGroupAvatar>
                        <div>
                      ...
                      <HandleGroupAvatar team={item.awayTeam} sport={SPORT.TENNIS} size={17}></HandleGroupAvatar>
                    </div>
                  </TwBorderLinearBox>
                </div>
              </div>
