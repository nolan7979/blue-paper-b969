...
export const QvPlayer = ({
  ...
  return (
    <div className=' relative flex items-center gap-2'>
      ...
      {activeTab && (
        <div className='absolute -top-4 left-3  h-min'>{renderOptional}</div>
      )}
      <div className='flex flex-1 justify-between'>
        <div className=''>
          <div className='flex flex-col'>
            ...
            <div className=' text-light-defaul flex items-center gap-x-2 text-csm font-normal'>
              {shirtNo > 0 ? `${shirtNo}`.padStart(2, '0') + ' - ' : ''}
              <div className='flex  justify-start gap-y-1 text-xss font-normal text-light-default dark:text-white'>
                {name}
              </div>
          ...
          {/* TODO out player */}
          {subIn && (
            <div className='flex items-center gap-2 text-xs font-normal text-light-default dark:text-dark-text'>
...
export const FormBadgeWithHover = ({
  ...
  const startDt = startTimestamp
    ...
    : '';
  const score = `${homeScore.display} - ${awayScore.display}`;
  const title: any = `${startDt && startDt + ':'} ${score}, ${
  ...
  return (
    <div
      onClick={(e) => {
        ...
        e.stopPropagation();
        window.open(
          `/match/${getSlug(homeTeam.name)}-${getSlug(awayTeam.name)}/${id}`,
          '_blank'
        );
      }}
      className={clsx(
...
export const SoccerTeam = ({
  ...
  return (
    ...
    >
      {showIcon &&
        (isLink ? (
          <Link href={`/competitor/${team?.slug || team?.name}/${team?.id}`}>
            <Avatar
              id={team?.id}
              type='team'
              width={logoSize}
      ...
      {showName && (
        <>
          <div
            test-id='team-name'
            className={`flex w-full items-center font-primary text-csm font-normal ${
              teamPlaying ? 'text-red-500' : ''
            }`}
          >
            {team?.name}
          </div>
...
export const QvLineupsPlayerReverse = ({
  ...
  return (
    <div className=' relative flex flex-row-reverse items-center gap-2'>
      <Avatar id={id} type='player' width={imgSize} height={imgSize} isSmall />
      {activeTab && (
        <div className='absolute -right-3 -top-4  h-min'>{renderOptional}</div>
      )}
