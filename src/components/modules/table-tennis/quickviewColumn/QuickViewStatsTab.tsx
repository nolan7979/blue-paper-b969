...
import { useMatchStatsData } from '@/hooks/useTableTennis';
...
export const StatsSection: React.FC<{
  ...
  const shouldRefetching = checkInProgessMatch(status?.code);
            ...
            <TwQuickViewSection className='space-y-5 p-4 text-sm'>
              {statisticsItems &&
                statisticsItems.map((item: any, idx: any) => {
                  const { fields, home = 0, away = 0, compareCode } = item;
                  return (
                    <StatsIndexRow
                      key={`${period}--${fields}-${idx}}`}
                      statistic={(stat && stat[fields]) || fields}
      ...
      {periods.map((period: any, index: number) => (
        ...
        >
          <button
            ...
            className={`h-full w-full rounded-full px-4 py-1 ${
              statsPeriod === period.period
                ? 'dark:bg-button-gradient bg-dark-button ' +
                  (statsPeriod === period.period
                    ? 'text-white'
                    : 'text-black dark:text-white')
                : 'bg-head-tab text-black dark:bg-dark-head-tab dark:text-white'
            }`}
...
export const StatsIndexRow = ({
  ...
  const viewPercent = (num: number) =>
    num < 1 && num !== 0 ? `${Math.round(Math.abs(num * 100))}%` : num;

  return (
    <div className='space-y-2 text-sm font-normal'>
      <div className='flex justify-between'>
        <ScoreStats
          score={viewPercent(homeScoreDisplay)}
          highlight={awayScoreDisplay < homeScoreDisplay}
          type='home'
        />
        ...
        <ScoreStats
          score={viewPercent(awayScoreDisplay)}
          highlight={awayScoreDisplay > homeScoreDisplay}
          type='away'
        />
      </div>
          ...
          <span
            ...
            style={{
              width: except
                ? `${homeScoreDisplay}%`
                : `${Math.round(
                    (Math.abs(homeScoreDisplay) / totalScore) * 100
                  )}%`,
            }}
          />
        </div>
          ...
          <span
            ...
            style={{
              width: except
                ? `${awayScoreDisplay}%`
                : `${Math.round(
                    (Math.abs(awayScoreDisplay) / totalScore) * 100
                  )}%`,
            }}
          />
        </div>
