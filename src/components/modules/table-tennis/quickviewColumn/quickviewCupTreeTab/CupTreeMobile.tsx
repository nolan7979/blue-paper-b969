...
import { MatchItemCupTree } from '@/components/modules/table-tennis/quickviewColumn/quickviewCupTreeTab/MatchItemCupTree';
  ...
  return (
    <div className="lg:no-scrollbar w-full flex place-content-center rounded-md p-4 overflow-x-scroll">
      ...
      <CustomModal open={openModal} setOpen={() => setOpenModal(!openModal)}>
        {listIdEvent.length > 0 ? listIdEvent.map((ev:any) => <MatchItemCupTree eventId={ev} key={ev} />) : <div>No events</div>}
      </CustomModal>
    </div>
  );
};
...
const Rounds = ({
  ...
  return (
    <div className="flex flex-col items-center relative">
      {(left?.order === 4 || left?.order === 8 || left?.order === 10 || right?.order === 5 || right?.order === 9 || right?.order === 11) ? (
        <div className="flex flex-col items-center relative">
          <div className="flex relative gap-[8px]">
            <div className="flex flex-col items-center relative">
              <Rounds roundData={left} firstOrder={firstOrder} maxView={maxView} parentOrder={parentOrder} matchClick={matchClick} />
              <div className="h-[12px] w-[1px] bg-dark-blue-border" />
            </div>

            <div className="flex flex-col items-center relative">
              <Rounds roundData={right} firstOrder={firstOrder} maxView={maxView} parentOrder={parentOrder} matchClick={matchClick} />
              <div className="h-[12px] w-[1px] bg-dark-blue-border" />
            </div>
          </div>
      ...
      <div className=''>
        {isFinal && (
          <div className="flex items-center">
            <span className="z-[1] absolute right-0 -left-[50px] bottom-1/2 top-[47%]">
              <ChampionSVG className="h-[40px] w-[40px]" />
            </span>
          </div>
        )}
        <div className="flex flex-row justify-center items-center relative" onClick={() => {matchClick(events)}}>
          <div className="flex flex-col justify-center items-center bg-custom-gradient-item-row px-[8px] py-[12px] rounded-lg relative">
            ...
            {leftParticipant?.team?.sub_ids && leftParticipant?.team?.sub_ids.length > 0 ? (
                <HandleGroupAvatar team={leftParticipant?.team} sport={SPORT.TABLE_TENNIS} size={20}></HandleGroupAvatar>
              ) : (
                <HandleGroupAvatar team={leftParticipant?.team} sport={SPORT.TABLE_TENNIS} size={20}></HandleGroupAvatar>
              )}
              {rightParticipant?.team?.sub_ids && rightParticipant?.team?.sub_ids.length > 0 ? (
                <HandleGroupAvatar team={rightParticipant?.team} sport={SPORT.TABLE_TENNIS} size={20}></HandleGroupAvatar>
              ) : (
                <HandleGroupAvatar team={rightParticipant?.team} sport={SPORT.TABLE_TENNIS} size={20}></HandleGroupAvatar>
              )}
            </div>
          ...
          {order === 1 && thirdPlaceCupBlock && (
            <div className="ml-4">
              <ThirdPlaceBlock thirdPlaceCupBlock={thirdPlaceCupBlock} setEvents={matchClick} />
            </div>
          )}
        </div>
      </div>
      ...
      {right?.order === 3 && (
        <><div className="h-[12px] w-[1px] bg-dark-blue-border" /><Rounds roundData={right} firstOrder={firstOrder} maxView={maxView} parentOrder={parentOrder} matchClick={matchClick} /></>
      )}
    ...
    <div
      ...
      onClick={() => {
        if (eventIds && eventIds.length > 0) {
          ...
          //   router.push(`/${SPORT.ICE_HOCKEY}/match/${eventIds[0]}`)
          // }
        }
      }}
    >
      ...
      <div className="flex justify-between items-center border-b border-gray-700 pb-2 mb-2">
        {leftTeam?.sub_ids && leftTeam?.sub_ids.length > 0 ? (
          leftTeam.sub_ids.map((subId: string) => (
            <HandleGroupAvatar team={leftTeam} sport={SPORT.TABLE_TENNIS} size={20}></HandleGroupAvatar>
          ))
        ) : (
          leftTeam?.id ? (
            <HandleGroupAvatar team={leftTeam} sport={SPORT.TABLE_TENNIS} size={20}></HandleGroupAvatar>
      ...
      <div className="flex justify-between items-center">
        {rightTeam?.sub_ids && rightTeam?.sub_ids.length > 0 ? (
          rightTeam.sub_ids.map((subId: string) => (
            <HandleGroupAvatar team={rightTeam} sport={SPORT.TABLE_TENNIS} size={20}></HandleGroupAvatar>
          ))
        ) : (
          rightTeam?.id ? (
            <HandleGroupAvatar team={rightTeam} sport={SPORT.TABLE_TENNIS} size={20}></HandleGroupAvatar>
