...
export const LiveOddsCompareTableRepr1x2 = ({
  matchId,
  half = 0,
}: {
  matchId: string;
  ...
  const { odds1x2CompareData, odds1x2CompareDataHT } = useOddsDetailsStore();
  ...
  const shownData = half === 0 ? odds1x2CompareData : odds1x2CompareDataHT;
...
const OddsDetailRow1x2 = ({
  ...
  const { odds1x2CompareData, odds1x2CompareDataHT } = useOddsDetailsStore();
  const bookData =
    (half === 0 ? odds1x2CompareData[bookId] : odds1x2CompareDataHT[bookId]) ||
    {};
  const { std1x2 } = bookData || {};
