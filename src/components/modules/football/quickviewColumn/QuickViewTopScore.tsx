...
const QuickViewTopScore = ({ matchData }: { matchData: SportEventDto }) => {
  ...
  if (data.topPlayers && data.topPlayers.goals) {
    return (
      <TwQuickViewSection className='dark:bg-dark-card lg:rounded-lg'>
        <div className='flex justify-between p-2.5  bg-head-tab dark:bg-dark-head-tab dark:text-dark-text lg:rounded-t-lg'>
        ...
        <ul>
          {data.topPlayers.goals.map((d: ITopPlayerItem, index: number) => (
            <li key={index} className='item-hover cursor-pointer px-2.5 border-b py-2 last:border-none dark:border-head-tab dark:bg-dark-card'>
              <PlayerStatsV2
...
export default memo(QuickViewTopScore, (prevProps, nextProps) => {
