...
import QuickViewDetailTab from '@/components/modules/football/quickviewColumn/quickviewDetailTab';

import { useMatchStore, useMatchStore2nd } from '@/stores';

import AdHorizentolComponent from '@/components/layout/AdHorizentolComponent';
...
import QuickviewCommentary from '@/components/modules/football/quickviewColumn/QuickviewCommentary';
...
import { QuickViewFilter } from '@/modules/football/liveScore/components/QuickViewFilter';
...
const QuickViewMatchesTab = dynamic(
  () =>
    import('@/components/modules/football/quickviewColumn/quickViewMatchesTab'),
  { ssr: false }
);
const QuickViewOddsTab = dynamic(
  () =>
    import('@/components/modules/football/quickviewColumn/QuickViewOddsTab'),
  { ssr: false }
);
const QuickViewSquadTab = dynamic(
  () =>
    import('@/components/modules/football/quickviewColumn/QuickViewSquadTab'),
  { ssr: false }
);
const QuickViewStandingsTab = dynamic(
  () =>
    import(
      '@/components/modules/football/quickviewColumn/QuickViewStandingsTab'
    ),
  { ssr: false }
);
...
const QuickViewTopScore = dynamic(
  () =>
    import('@/components/modules/football/quickviewColumn/QuickViewTopScore'),
  { ssr: false }
);
const QuickViewFootyTab = dynamic(
  () =>
    import('@/components/modules/football/quickviewColumn/quickviewFootyTab'),
  { ssr: false }
);
...
export const QuickViewColumn = ({
  ...
  const { matchLiveSocket } = useMqttClient(SPORT.FOOTBALL, !!isMobile);
  ...
  const { data } = useSelectedMatchData(
    !isDetail ? matchId || selectedMatch || featureMatchId || '' : '',
  ...
  const matchShowData = useMemo(() => {
    ...
    const isMatchEnded = (data && [MatchState.AET, MatchState.End, MatchState.AP].includes(data?.status?.code))
    const checkMatchCurrentTime = isMatchEnded ? data?.time : matchDetails?.time;

    const matchDataMapping = {
      ...data,
      ...matchDetails,
      ...extraHasData(data as SportEventDtoWithStat),
      status:  matchDetails?.status.code === -1 ? data?.status : matchDetails?.status,
      time: checkMatchCurrentTime,
    };

    // handle data detail when match is live
    if (matchLiveSocket && matchLiveSocket?.id === matchData?.id && isDetail) {
      const { metadata, payload } = matchLiveSocket;
      const compareMatch = metadata?.sport_event_id == matchData?.id;
      ...
      const updatedMatch = {
        ...
        status: convertStatusCode(
          matchStateIdToMatchState[
          Number(status_id) as keyof MatchStateIdToMatchState
          ]
        ),
        time: {
    ...
    if (!selectedMatch) {
      if (!isValEmpty(featureMatchdaily)) {
        return featureMatchdaily;
      } else {
        return {
          ...(isDetail ? matchData : firstMatch),
          ...extraHasData(data as SportEventDtoWithStat),
          status: isDetail ? matchData?.status : firstMatch?.status,
        };
      }
    }
  ...
  const hasAds = useMemo(() => {
    return !domain.includes('.com') && isMobile;
  }, [domain, isMobile]);
  ...
  useEffect(() => {
    if (activeRating) {
      setActiveTab(activeRating);
    }
  }, [activeRating]);

  useEffect(() => {
    const isDiffrentLocale =
      router.locale && !supportedLocales.includes(router.locale);
    const changeLocaleWhenActiveCommentary =
      isDiffrentLocale && activeTab === 'commentary';
    if (changeLocaleWhenActiveCommentary) {
      setActiveTab('details');
    }
  }, [router.locale]);
        ...
        <div className='space-y-4'>
          <div className='sticky top-[3.313rem] z-[11] lg:relative lg:top-0'>
            ...
            <QuickViewFilter
              ...
              hasCommentary={isDisplayCommentary}
          ...
          {activeTab === 'chat' && (
            <QuickViewChatTab sport={SOCKET_CHAT_TOPIC.football} />
          )}

          {activeTab === 'commentary' && isDisplayCommentary && (
            <QuickviewCommentary
              matchData={matchShowData as SportEventDtoWithStat}
            />
          )}
      ...
      <div className='h-full space-y-4 overflow-y-scroll no-scrollbar'>
        <div className='flex justify-between '>
          <div className=' flex items-center gap-2 overflow-hidden text-xs font-extralight'>
            ...
            <BreadCumbLink
              href={`/football/competition/${tournament.slug}/${tournament?.id || tournament?.id
                }`}
...
export const TwQuickviewTeamName = tw.div`text-center text-[11px] font-bold dark:text-white lg:text-dark-dark-blue lg:dark:text-white line-clamp-3 overflow-hidden text-ellipsis`;
