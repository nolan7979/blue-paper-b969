...
const RatingSection = memo(({
  ...
  if (isValEmpty(data) || parseInt(data?.bestHomeTeamPlayer?.value) === 0 || parseInt(data?.bestAwayTeamPlayer?.value)  === 0) {
    return <></>;
  }
  ...
  return (
    <TwBorderLinearBox
      className={`${!isDetail ? 'dark:border-linear-box bg-white dark:bg-primary-gradient' : ''
        }`}
    >
      <InfoHomeAndAwayTeam
        infoHome={homePlayer}
  ...
  return prevProps.matchData?.id === nextProps.matchData?.id
});
