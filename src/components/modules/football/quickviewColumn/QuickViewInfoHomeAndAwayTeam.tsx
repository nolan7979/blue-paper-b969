...
export const InfoHomeAndAwayTeam: React.FC<IInfoHomeAndAwayTeam> = ({
  ...
  const renderValue = useMemo(() => {
    if (isH2H) {
      const parseContentValue = (value: React.ReactNode) =>
        parseInt(value as string) || 0;

      const total =
        parseContentValue(content?.start) +
        parseContentValue(content?.middle) +
        parseContentValue(content?.end);

      const calculatePercentage = (value: number) =>
        total === 0 ? 0 : Math.round((value / total) * 100);

      const homeWin = calculatePercentage(parseContentValue(content?.start));
      const awayWin = calculatePercentage(parseContentValue(content?.end));
      const draw = total === 0 ? 0 : 100 - homeWin - awayWin;
      return (
        <>
      ...
      {(isH2H && (
        <div className='space-y-3.5 p-2.5'>
          <div className='flex items-center justify-between' test-id='logo-row'>
            <CustomLink
              href={`/football/${renderType}/${infoHome.slug}/${infoHome?.id}`}
              target='blank'
            >
              <Avatar
                id={infoHome?.id}
            ...
            <CustomLink
              href={`/football/${renderType}/${infoAway.slug}/${infoAway?.id}`}
              target='blank'
            >
              <Avatar
                id={infoAway?.id}
