...
import ClubSVG from '/public/svg/club.svg';
...
const QuickViewSquadTab: React.FC<{ matchData: SportEventDto }> = ({
  ...
  }, [refetch, shouldRefetch, matchData?.id]);
  ...
  const isNationalTeam = useMemo(() => {
    return (home?.players?.length === 0 ? away : home)?.isNationalTeam;
  }, [home, away]);
    ...
    <>
      <div>
        ...
        <TwTabHead className='mb-3.5 mt-4'>
          {Object.keys(LINEUPS_TAB).map((item: any, index) => {
            return (
              <TwBorderLinearBox
                key={item}
                className={` h-full w-1/3 !rounded-full ${
                  activeTab === item
                    ? 'dark:border-linear-form bg-active-squad text-white'
                    : ''
                }`}
              >
                <button
                ...
                >
                  ...
                  <span className='text-csm font-medium dark:text-white'>
                    {isNationalTeam && item === LINEUPS_TAB.nationality
                      ? i18n.menu.team
      ...
      {/* ĐỘI HÌNH RA SÂN */}
      <div className='space-y-2 px-2.5 dark:lg:px-0 bg-white dark:bg-transparent pb-4' test-id='Substitute'>
        {confirmed ? (
        ...
        <div className='relative grid grid-cols-2 items-start justify-center gap-x-6 mb-6'>
          {/* Home team */}
          <TeamPlayerList
            team={homeTeam}
            activeTab={activeTab as keyof typeof LINEUPS_TAB}
...
export const TeamPlayerList = ({
  ...
  for (const playerData of teamLineups.players) {
    ...
    mapPlayerRating[player?.id] = statistics.rating || Number(rating);
  }

  return (
    <div className='' test-id='home-line-up'>
      <div
        className={clsx(
      ...
      >
        ...
        />
        <div
          className='truncate text-csm font-semibold dark:text-white'
          test-id='team-name'
        >
          {team.name}
        </div>
      ...
      <ul className=' divide-list divide-y' test-id='list-player-home'>
...
export const PlayerLineUp = ({
  ...
  return (
    <div
      className='relative flex flex-col items-center text-xxs xl:text-cxs flex-1'
    ...
    >
        ...
        <div className='absolute top-0 ml-2 flex flex-col justify-between'>
          {activeType && (
            <div
              ...
              className={`absolute left-2 top-0 z-10 rounded-md  px-1 text-black dark:text-white ${
                activeType !== 'nationality' ? ' bg-white dark:bg-black' : ''
              }`}
            >
              <>{renderOptional}</>
            </div>
      ...
      >
        ...
        >
          <span className={`pr-0.5 text-msm font-medium text-center whitespace-normal leading-[1.5] w-full lg:max-w-full ${lineLength > 3 ? 'max-w-18' : 'max-w-26' }`} test-id='name-player'>
            {captain && '(C) '}
            {name}
          </span>
        </Tippy>
...
export const PlayerCompetitionLineUp = ({
  src,
  name,
  shirtNo,
  isHome = true,
  ...
  interface TruncatedTextProps {
   ...
   const TruncatedText: React.FC<TruncatedTextProps> = ({ text }) => {
      ...
      return (
        <span className="block w-full text-msm font-medium text-center flex-wrap text-white dark:text-dark-default">
           {captain && '(C) '}
          {truncatedText}
        </span>
      );
  ...
  return (
    ...
    >
      <div className='flex items-center justify-center'>
        <div
          className='absolute    z-[2] mr-[2.3rem] mt-3 flex h-auto w-auto justify-end'
          test-id='sub-line'
        >
          {!!rating && Number(rating) > 0 && (
            <div className='h-5/12 relative'>
        ...
        <div className='absolute mt-4 ml-[35px] flex flex-col justify-between'>
          <div className='h-5/12 flex' test-id='numGoals'>
            {team?.id && (
              <Avatar
                id={team?.id}
      ...
      <div
        className={`flex  ${lineLength > 2 ? 'w-full' : 'w-fit px-3'} items-center space-x-0.5 rounded-cmd bg-black/20  text-sm `}
        test-id='search-no'
      >
        {shirtNo && (
          <span
        ...
        <Tippy
          ...
          placement='bottom-end'
        >
          {/* <span className={`pr-0.5 text-msm font-medium text-center block w-full h-[20px] overflow-hidden text-ellipsis whitespace-normal `} test-id='name-player'>
...
export const VerticalHomeTeamLineUp = ({
  ...
  return (
    <div className='relative w-full bg-cover bg-no-repeat h-full'>
      <div className='flex flex-col place-content-center gap-y-2 h-full justify-evenly'>
...
export const VerticalCompetiotionTeamLineUp = ({
  lineups,
  mapPlayerEvents,
  matchData,
  showStats = true,
  ...
  return (
    <div  style={{
      backgroundImage: "url('/svg/Group.svg')",

    }} className='relative flex justify-center md:bg-contain lg:bg-cover CustomResponeSive bg-cover w-full mt-4  bg-no-repeat bg-center  h-[280px]'>
      <div className='absolute flex flex-col xs:px-10 px-4 lg:px-6 md:px-16 w-full lg:w-full  md:w-[56%] place-content-center  h-full justify-start mt-[-0.35rem] gap-[0.35rem] top-0'>
        {linePlayers.map((lineup: ILineupsPlayer[], index: number) => {
          return (
            <div
              key={`line-${index}`}
              className={`grid grid-cols-${lineup.length} gap-[0.2rem] flex-wrap items-start justify-evenly `}
            >
              {lineup.map((playerLineup: any, index: number) => {
                const {
                  team = {},
...
export const TwLineupLineAway = tw.div`flex flex-row-reverse justify-evenly items-center`;
...
export const VerticalAwayTeamLineUp = ({
  ...
  return (
    <div className='w-full rounded-b-md bg-cover bg-no-repeat h-full'>
      <div className='flex flex-col place-content-center gap-y-2 h-full justify-evenly items-center'>
        {linePlayers
          .slice()
          .reverse()
          .map((lineup: any, index: number) => {
            return (
              <TwLineupLineAway
                key={`line-${index}`}
                className='flex-wrap items-center justify-evenly w-full'
              >
...
export const InjuriesSuspensionSection = ({
  ...
  return (
    <TwMbQuickViewWrapper className={clsx(className, 'mt-5')}>
      <TwQuickViewTitleV2 className='py-2 text-center'>
