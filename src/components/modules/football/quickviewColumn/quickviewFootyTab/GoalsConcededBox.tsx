...
import { GoalsConcededProps } from '@/components/modules/football/quickviewColumn/quickviewFootyTab/GoalsConceded';

const GoalsConcededBox: FC<GoalsConcededProps> = ({
  homeTeam,
  awayTeam,
  data,
}) => {
  ...
  const percentageWinner = useMemo(() => {
    const { homePercentage } = calculatePercentage(
      data?.seasonConcededAVGHome || 0,
      data?.seasonConcededAVGAway || 0
    );
    return homePercentage;
  }, [data]);

  const subContent = useMemo(() => {
    const homePPgFormatted = Number(data?.seasonConcededAVGHome) || 0;
    const awayPPgFormatted = Number(data?.seasonConcededAVGAway) || 0;

    if (homePPgFormatted === awayPPgFormatted) {
      return homePPgFormatted === 0 ? '' : i18n.footy_stats.equal_conceded;
    }

    if (homePPgFormatted === 0) {
      return i18n.footy_stats.advantage_conceded.replace(
        ':teamName',
    ...
    if (homePPgFormatted > awayPPgFormatted) {
      return i18n.footy_stats.team_better_conceded
    ...
    <WrapperBorderLinearBox className='mt-[10px] flex flex-col gap-4 p-3'>
      <div className='flex w-full justify-around gap-4'>
        <div className='flex flex-col items-center gap-1'>
          ...
          />
          {data?.seasonConcededAVGHome && (
            <span className='text-base font-semibold dark:text-white'>
              {data?.seasonConcededAVGHome || ''}
            </span>
          )}
          <span className='text-csm'>{i18n.footy_stats.per_game_conceded}</span>
        </div>
        <div className='flex flex-col items-center gap-1'>
          <Avatar
            type='team'
...
export default GoalsConcededBox;
