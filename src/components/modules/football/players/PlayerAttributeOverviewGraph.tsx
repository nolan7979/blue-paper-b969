...
export default function PlayerAttributeOverviewGraph({
  ...
  if (isComparing) {
    ...
    if (yearShift === 0) {
      ...
      refName = calShiftYear(yearShift, i18n);
      viewName = calShiftYear(0, i18n);
    }
  }
      ...
      <div className='attribute flex flex-col items-center text-sm text-dark-default'>
        <div className=' relative flex h-72 w-72 justify-center'>
          ...
          <AttributeBadgeDouble
            ...
            yearShift={yearShift}
            top='top-26'
            left={sortedData[4]?.refValue === sortedData[4]?.attributeValue ? '-left-10' : '-left-18'}
            transform={
...
const CustomLegend = (props: any) => {
  const { payload } = props;
  const payloadFiltered = mergeData(payload);
  return (
    <ul
      style={{ margin: 0, padding: 0, listStyle: 'none' }}
      className='flex place-content-center items-center gap-4 text-xs'
    >
      {Array.isArray(payloadFiltered) && payloadFiltered.map((entry: any, index: number) => (
        <li key={`item-${index}`}>
...
export const calShiftYear = (yearShift: number, i18n: any) => {
...
function mergeData(dataArray: any) {
  return Object.values(
    dataArray.reduce((acc: any, item: any) => {
      ...
      if (!acc[key]) {
        ...
        acc[key] = {
          ...acc[key],
          ...item,
          payload: { ...acc[key].payload, ...item.payload }
        };
      }
      return acc;
    }, {})
