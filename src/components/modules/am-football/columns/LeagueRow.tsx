...
export const LeagueRow = ({
  ...
  const { uniqueTournament: tournament } = match;
  const { category = null } = tournament || {};
  ...
  const isTournamentFollowed = (
    ...
    const tournamentSport = followedTournaments['am-football'] || [];
    return tournamentSport.some((item: any) => item?.id === tournament?.id);
  };
  const [isFollowedTour, setIsFollowedTour] = useState(() =>
    isTournamentFollowed(tournament as TournamentDto, tournamentFollowed)
  );
  ...
  const changeFollow = useCallback(() => {
    if (!isFollowedTour) {
      addTournament('am-football', newTournament);
    } else {
      removeTournament('am-football', newTournament);
    }
    // if(session && Object.keys(session).length > 0) {
    //   const dataFavoriteId = {
    //     id: tournament?.id,
    //     sportType: getSportType(SPORT.AMERICAN_FOOTBALL),
  ...
  const buildCompetitionLink = useCallback(
    () =>
      `/am-football/competition/${tournament?.slug}/${
    ...
    <div className={containerClass} test-id='match-row'>
      ...
      <div className='flex w-12 items-center justify-end'>
        <CustomLink
          ...
          className='dsfsd hover:text-logo-blue'
          target='_parent'
        >
          <Avatar
        ...
        {(category?.name && category?.id && (
          <div className='text-msm dark:text-dark-text'>
            {isCountryName(category.name) && (
              <CustomLink
                disabled
                href={`/am-football/country/${getSlug(category?.name)}/${
                  category?.id
                }`}
          ...
          <div className='text-csm dark:text-dark-text' test-id='country-name'>
            <span>-</span>
          </div>
        )}
        <CustomLink
