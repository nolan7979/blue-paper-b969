...
export const useMatchTimestamp = (
  startTimestamp: number,
  currentPeriodStartTimestamp: number | any,
  status: IStatusCode,
  id: string,
  sport: string,
  ...
  const [dateTimeZone, setDateTimeZone] = useState(() => {
    const [dateStr] = formatMatchTimestampBaseball(
      startTimestamp,
      status || {},
      false,
  ...
  const [timeTimeZone, setTimeTimeZone] = useState(() => {
    const [, timeStr] = formatMatchTimestampBaseball(
      startTimestamp,
      status || {},
      false,
  ...
  useEffect(() => {
    const updateTimestamps = () => {
      const [newDateStr, newTimeStr] = formatMatchTimestampBaseball(
        startTimestamp,
        status,
        false,
...
const MatchTimestamp = ({
  ...
  const isFinished = [MatchBaseballState.ENDED].includes(status.code);
  const isAbnormal = [MatchBaseballState.ABNORMAL].includes(status.code);
  const isPostponed = [MatchBaseballState.POSTPONED].includes(status.code);
        ...
        <TwTimeCol dir='ltr' className='flex h-full flex-col gap-1'>
          ...
          <span
            className={clsx(
              ...
              isInprogress || isFinished || isPostponed
