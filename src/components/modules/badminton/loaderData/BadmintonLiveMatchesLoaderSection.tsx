...
export const BadmintonLiveMatchesLoaderSection = () => {
...
const LiveMatchesLoader = () => {
  ...
  const {
    ...
    refetch,
    isLoading,
  } = useLiveMatchData(SPORT.BADMINTON);

  useEffect(() => {
    if (liveMatches) {
      const liveMatches_convert = parseMatchDataArrayBadminton(liveMatches as string);
