...
import { TwMobileView } from '@/components/modules/football/tw-components';
import {
  TwBorderLinearBox,
  TwTabHead,
} from '@/components/modules/football/tw-components/TwCommon.module';
import { useMatchStatsData } from '@/hooks/useBadminton';
...
export const StatsSection: React.FC<{
  ...
  const shouldRefetching = isMatchBmtLive(status?.code);
  ...
  if (isValEmpty(statisticsItems)) {
    return (
      <>
        <EmptyEvent content='No data available'></EmptyEvent>
      </>
    );
  }
        ...
        <TwMbQuickViewWrapper className='space-y-3' id='stats'>
          <div className='space-y-3 px-2.5 lg:px-0'>
            ...
            <div className='space-y-5 rounded-md bg-white p-4  text-sm dark:bg-dark-card'>
              {statisticsItems &&
                statisticsItems.map((groupData: any) => {
                  const rows: any[] = [];
...
export const StatsIndexRow = ({
  ...
  const isDisplayPercentage =
    fieldKey &&
    ['service_points_rate', 'receiver_points_rate'].includes(fieldKey);
  const except =
