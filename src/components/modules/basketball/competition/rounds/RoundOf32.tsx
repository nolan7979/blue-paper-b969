...
import { MatchItemCupTree } from '@/components/modules/basketball/competition/MatchItemCupTree';
...
const BracketPages = ({
  ...
  const finalRounds = ['Final', 'Finals'];
  const isFinalRound = finalRounds.includes(activeTab || '');
...
const Rounds = ({
  ...
  parentOrder?: number;
  root?: boolean;
  activeTab?: string; //Todo: improve to hyphenated Key
  activeIndex?: number;
  totalTabs?: number;
  handlePrev?: () => void;
  handleNext?: () => void;
  ...
  const renderTeamAvatar = (teamId: number = 0, item: any) => {
    return teamId ? (
      <>
        {/* <HandleGroupAvatar team={item} sport={SPORT.BASKETBALL} size={25}></HandleGroupAvatar> */}
        <Avatar
          id={teamId.toString()}
          width={25}
          height={25}
  ...
  const renderMatch = (item: any) => {
    ...
    return (
      <TwFullBlockContainer
        className="relative my-1.5 w-full cursor-pointer !border-none dark:bg-gradient-to-br bg-white"
        onClick={() => {
      ...
      >
        <div className="flex gap-2 p-2">
          ...
          <div className="flex max-w-fit flex-col text-black dark:text-white">
            <p className={clsx('flex-1 truncate leading-6', { 'font-extrabold': item.homeTeamScore > item.awayTeamScore })}>
    ...
    <div className="relative flex items-center" key={groupIndex}>
          ...
          {group.map((item: any, subIndex: number) => (
              <div key={`left-${groupIndex}-${subIndex}`} className='relative flex items-center' >
                {activeTab !== 'Quarter Finals' && (
                  <>
                    <button
                      className='absolute left-[4px] z-10 flex h-[32px] w-[32px] items-center justify-center rounded-full bg-white hover:bg-gray-700 dark:bg-gray-800'
                      onClick={handlePrev}
                    >
                      <IcPrev />
                    </button>
      ...
      {!isFinalRound && (
        <>
          <button
            onClick={handleNext}
            className="absolute right-[13px] flex items-center justify-center w-[32px] h-[32px] rounded-full bg-white dark:bg-gray-800 hover:bg-gray-700"
          >
            <IcNext />
          </button>
        ...
        {isFinalRound && participants ? (
          <div className="relative flex items-center">
            {activeTab !== 'R32' && (
              <>
                <button
                  className="absolute left-[4px] z-10 flex items-center justify-center w-[32px] h-[32px] rounded-full bg-white dark:bg-gray-800 hover:bg-gray-700"
                  onClick={handlePrev}
                >
                  <IcPrev />
                </button>
