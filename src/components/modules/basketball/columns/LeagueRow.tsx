...
export const LeagueRow = ({
  ...
  const buildCompetitionLink = useCallback(
    () =>
      `/basketball/competition/${tournament?.slug}/${
  ...
  return (
    <div className={containerClass} test-id='match-row'>
      ...
      <div
        className='flex flex-col items-start truncate leading-normal  md:w-full md:flex-1'
        ...
        {category?.name && (
          <div className='text-msm dark:text-dark-text'>
            {isCountryName(category?.name) && (
              <CustomLink
                href={`/basketball/country/${getSlug(category?.name)}/${
                  category?.id
                }`}
                className='hover:text-logo-blue'
                disabled={true}
