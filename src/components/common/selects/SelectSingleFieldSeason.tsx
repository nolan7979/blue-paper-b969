...
export function SelectSingleFieldSeason({
  options,
  label = 'name',
  size = 'full',
}: // initialSelectedProps,
{
  ...
  initialSelectedProps?: any;
}) {
  const initialSelected = options[0];
  const [selectedOption, setSelected] = useState(initialSelected);
  const { selectedStandingsSeason, setSelectedStandingsSeason } =
    useTeamStore();

  useEffect(() => {
    // if (initialSelectedProps) {
    //   setSelected(initialSelectedProps);
    // }
    setSelectedStandingsSeason(selectedOption);
  }, [selectedOption, setSelectedStandingsSeason]);
        ...
        <div className='relative rounded-lg bg-light-match dark:bg-dark-head-tab'>
          <Listbox.Button className='dev relative h-8 w-full cursor-pointer rounded-lg px-3 py-1 text-left shadow-sm focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2  focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 dark:bg-dark-head-tab sm:text-sm'>
            <div className='flex w-full items-center justify-start gap-x-2'>
              <CalendarSVG className='h-[13px] w-[14px] text-light-default dark:text-dark-text' />
              <span className='block truncate text-center text-xs dark:text-white'>
                {selectedStandingsSeason['year']}
              </span>
            </div>
